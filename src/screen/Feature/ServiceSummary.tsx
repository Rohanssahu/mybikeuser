...
const STATUS_CFG: Record<string, {color: string; bg: string; label: string}> = {
  ...
  rejected: {
    color: '#EF4444',
    bg: 'rgba(239,68,68,0.15)',
    label: 'Rejected by Service Center',
  },
  expired: {
    color: '#EF4444',
    bg: 'rgba(239,68,68,0.15)',
...
const ServiceSummary: React.FC<any> = ({navigation}) => {
  ...
  const [pickupAddress, setPickupAddress] =
    useState<string>('Fetching address…');
  ...
  const status =
    rawStatus === 'expired' || dealerResponseStatus === 'expired'
      ? 'expired'
      : rawStatus;
  ...
  const hasCoords =
    !isNaN(shopLat) && !isNaN(shopLng) && shopLat !== 0 && shopLng !== 0;

  const showCashConfirmed =
    status === 'payment_selected' && booking?.paymentMethod === 'CASH';
        ...
        <ScrollView
          ...
          <View style={styles.card}>
            ...
            {[
              ...
              {
                label: 'Last Service',
                value: booking?.lastServiceKm
                  ? `${booking.lastServiceKm} km`
                  : '—',
              },
              {
                label: 'Pickup',
          ...
          <View style={styles.billCard}>
            ...
            <View style={styles.statusGrid}>
              <View style={styles.statusGridItem}>
                ...
                <View
                  style={[
                    styles.statusGridBadge,
                    {backgroundColor: statusCfg.bg},
                  ]}>
                  <Text
                    style={[styles.statusGridVal, {color: statusCfg.color}]}>
                    {statusCfg.label}
                  </Text>
                </View>
              </View>
              <View style={styles.statusGridItem}>
                ...
                <View
                  style={[
                    styles.statusGridBadge,
                    {backgroundColor: billStatusCfg.bg},
                  ]}>
                  <Text
                    style={[
                      styles.statusGridVal,
                      {color: billStatusCfg.color},
                    ]}>
                    {billStatusCfg.label}
                  </Text>
                </View>
          ...
          <Text style={styles.footer}>
            Thank you for choosing <Text style={styles.footerHL}>MR BIKE!</Text>
            {'\n'}Ride Safe! 🏍️
          </Text>
        </ScrollView>
      </SafeAreaView>
