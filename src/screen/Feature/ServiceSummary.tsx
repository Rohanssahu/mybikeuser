...
const ServiceSummary: React.FC<ServiceSummaryProps> = ({navigation}) => {
  ...
  const get_additionalservices = async () => {
    ...
    let apiId = booking?.dealer_id?._id;
    const digitsOnly = booking?.userBike_id?.bike_cc.replace(/\D/g, '');

    const state = await additionalservices(apiId, '', digitsOnly);

    if (state?.success) {
      // Already selected IDs from bookingData
      const selectedIds = booking?.additionalServices?.map((s: any) => s) || [];

      // Filter only those services whose id exists in bookingData.additionalServices
      const filtered = state.data.filter((s: any) =>
        selectedIds.includes(s._id),
      );

      setAddiServices(filtered);
    }
  };
        ...
        <View style={[styles.card, {marginTop: 10}]}>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                openGoogleMaps(
                  booking?.dealer_id?.latitude,
                  booking?.dealer_id?.longitude,
                  booking?.dealer_id?.shopName,
                );
              }}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>
              {booking?.userBike_id?.plate_number}
            </Text>
          </View>
          <View style={styles.row}>
          ...
          <Text style={styles.footerText}>
            ...
            <Text style={styles.highlight}>MR BIKE!</Text>
            {'\n'}Ride Safe!
          </Text>
        </View>
