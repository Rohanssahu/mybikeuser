...
const SelectLocation = () => {
    ...
    const handleSaveAddress = useCallback(async (address) => {
        try {
    ...
    const handleSelectLocation = useCallback(async (details) => {
        try {
            const { lat, lng } = details.geometry.location;
        //    _update_location(lat, lng);
            setLocation({ latitude: lat, longitude: lng, latitudeDelta: 0.01, longitudeDelta: 0.01 });
    ...
    const renderLocationItem = useCallback(({ item }) => {
        ...
        return (
            <TouchableOpacity onPress={() => handleSaveAddress(item)} style={styles.locationItem}>
                <View>
                    <Image source={icon.pin} style={{ height: 15, width: 15, tintColor: '#081041' }} />
    ...
    return (
        <View style={styles.container}>
            ...
            >
                <Image source={icon.pin} style={{ height: 30, width: 30, tintColor: '#fff' }} />
            </TouchableOpacity>
        </View>
    );
};
...
const styles = StyleSheet.create({
    ...
    locationItem: { padding: 15, borderBottomWidth: 1, borderBottomColor: '#ddd', flexDirection: 'row', alignItems: 'center' },
    locationName: { fontSize: 16, fontWeight: 'bold' },
    locationAddress: { fontSize: 14, color: '#555' },
    currentLocationButton: { position: 'absolute', bottom: 30, right: 30, backgroundColor: '#081041', borderRadius: 30, height: 60, width: 60, justifyContent: 'center', alignItems: 'center' },
});
